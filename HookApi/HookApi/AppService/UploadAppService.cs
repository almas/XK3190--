...
using RestSharp;
using SqlSugar;
using JsonConverter = SqlSugar.JsonConverter;

namespace HookApi.AppService
{
            ...
            try
            {
                ...
                using (var db = SugarDao.GetInstance())
                {
                    do
                    {
                        ...
                        var batchs = db.Queryable<Batches>().Where(s => s.upload == false).OrderBy(s => s.yearNum).Take(UploadPerSize).ToList();
                        if (batchs.Any())
                        {
